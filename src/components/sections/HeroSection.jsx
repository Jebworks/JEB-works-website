...
import {
  // All icons removed as they are no longer used in the JSX
} from 'lucide-react';
...
const HeroSection = ({ sectionRef, counter, scrollToSection }) => {
  const stats = [
    ...
    { value: "₹100Cr+", label: "Money Managed" /*, icon: Coins*/ }, // Removed
    { value: "4.9/5", label: "User Rating" /*, icon: Star*/ }, // Removed
  ];

  const features = [
    { /*icon: Shield,*/ text: "Bank-Grade Security" }, // Removed
    { /*icon: Zap,*/ text: "Instant Transactions" }, // Removed
    { /*icon: BarChart3,*/ text: "Smart Analytics" }, // Removed
    { /*icon: Smartphone,*/ text: "Mobile First" } // Removed
  ];
...
>
      <div className="max-w-7xl mx-auto text-center relative z-10 pt-20">
        ...
        <div className="flex flex-wrap justify-center gap-6 mb-12 animate-slide-up" style={{ animationDelay: "0.3s" }}>
          {features.map((feature, idx) => (
            <div key={idx} className="flex items-center gap-2 glass-effect px-4 py-2 rounded-full">
              {/* <feature.icon size={16} style={{ color: "var(--color-primary)" }} /> // Icon Removed */}
        ...
        >
          ...
          >
            {/* <Download size={24} className="text-white" /> // Icon Removed */}
        ...
        >
          {stats.map((stat, idx) => (
            <div key={idx} className="text-center group">
              {/* Icon Div Removed */}
              <div className="text-3xl md:text-4xl font-black text-gradient mb-2 pt-4"> {/* Added padding-top */}
                {stat.value}
              </div>
              <div
                className="text-sm"
